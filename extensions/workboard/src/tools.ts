 ...
 *
 * Modes via `OPENCLAW_WORKBOARD_REQUIRE_PROOF`: `enforce` (default), `warn`
 * (allow, and record the same unknown-status entry so the would-be refusals are
 * visible), `off` (legacy). An unrecognized value resolves to `enforce`.
 *
 * The default was `warn` for one build (2026-08-04) and moved to `enforce` the
 * same day. The shadow period was carried by an env-file override rather than
 * by the default, and that file is regenerated by `openclaw service install` —
 * so the override would have silently expired and taken the gate with it. The
 * safe state belongs in the code, where a reinstall cannot undo it.
 *
...
function requireProofMode(env: NodeJS.ProcessEnv = process.env): "enforce" | "warn" | "off" {
  ...
  // Unrecognized values resolve to `enforce`, not to the loosest mode. A typo
  // in an env file should not quietly reopen the door — the failure this gate
  // exists to prevent is silent, so its own misconfiguration must not be.
...
function noProofEntry(reason: string): ProofLike {
  return {
    status: "unknown",
    label: "completed without verifiable evidence",
    note: reason.trim(),
  };
}
    ...
    },
    {
      ...
      execute: async (_toolCallId, rawParams) => {
        ...
        return runClaimedCardMutation(rawParams, (id, record, scope) => {
          const input = record as { proof?: ProofLike; noProofReason?: unknown };
