...
export const ja_JP: TranslationMap = {
  ...
  newSession: {
    ...
    draft: "下書き",
    draftDescription: "公開するまでこのスレッドを自分だけのものにしておきます",
  ...
  dreaming: {
    ...
    toggleConfirmation: {
      subtitle: "Dreaming はグローバル設定です。このエージェントに限定されるものではありません。",
      enableTitle: "すべてのエージェントで Dreaming を有効化",
      enableDetail:
        "夜間の Dreaming スイープが、設定済みのすべてのエージェントワークスペースにわたって実行され、短期の recall を長期記憶へと昇格させます。この設定はすぐに適用されます。",
      enableConfirm: "Dreaming を有効化",
      disableTitle: "すべてのエージェントで Dreaming を無効化",
      disableDetail:
        "夜間の Dreaming スイープが、このエージェントだけでなく設定済みのすべてのエージェントで停止します。すでに書き込まれた記憶は残りますが、新しく昇格されるものはありません。この設定はすぐに適用されます。",
      disableConfirm: "Dreaming を無効化",
      saving: "保存中…",
