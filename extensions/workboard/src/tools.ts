 ...
 *
 * Modes via `OPENCLAW_WORKBOARD_REQUIRE_PROOF`: `enforce`, `warn` (allow, and
 * record the same unknown-status entry so the would-be refusals are visible),
 * `off` (legacy). Default `warn` for the shadow rollout; flip to `enforce`
 * once a few cycles have been observed.
 *
...
function requireProofMode(env: NodeJS.ProcessEnv = process.env): "enforce" | "warn" | "off" {
  const raw = String(env.OPENCLAW_WORKBOARD_REQUIRE_PROOF ?? "warn").toLowerCase();
